...
function validateRoleProfile(input) {
    ...
    if (key === 'custom' || key === 'deepseek') {
        if (!description)
            throw Object.assign(new Error('使用自定义角色说明时，请填写说明内容。'), { status: 400 });
        return;
    }
...
async function syncRolePreset(role) {
    try {
...
router.post('/role-suggestions', aiRateLimit, async (req, res) => {
    ...
    if (!body.preferenceSetName) {
        ...
        if (focusPreset)
            return res.json({ content: focusPreset.defaultPreference, source: 'preset' });
    }
...
router.post('/contacts', async (req, res) => {
    ...
    const created = await repo.createContact(body);
...
router.put('/contacts/:id', async (req, res) => {
    ...
    const updated = await repo.updateContact(id, body);
    if (!updated)
        return res.status(404).json({ error: 'Contact not found' });
...
router.post('/generate', aiRateLimit, async (req, res) => {
    ...
    for (const contactId of body.contactIds) {
        ...
        const generationRecordId = await repo.createGenerationRecord(body.inputRecordId ?? null, contact.id, role.key, content);
