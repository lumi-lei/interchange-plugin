...
export function slugifyPresetId(label) {
...
export async function writeRolePreset(role) {
    if (!role.dsgEnabled) {
    ...
    for (const name of role.dsgSkills) {
        if (await copySkill(name, presetSkillsDir))
            copiedSkills += 1;
        else
...
export async function removeRolePreset(roleKey) {
...
export async function regenerateAll(roles) {
    let regenerated = 0;
