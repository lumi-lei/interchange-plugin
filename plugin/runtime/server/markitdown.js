...
const missingCliMessage = "MarkItDown CLI is not available. Install it with pip install 'markitdown[all]' or configure MARKITDOWN_COMMAND.";
function truncateDiagnostic(text, tempDir = '') {
...
function fileNameForInput(input) {
    const candidate = input.originalname ?? input.filename ?? 'upload';
    return path.basename(candidate) || 'upload';
}
function runMarkItDown(command, inputPath, outputPath) {
    return new Promise((resolve, reject) => {
        execFile(command, [inputPath, '-o', outputPath], {
            encoding: 'utf8',
...
function classifyExecFailure(failure, tempDir) {
...
async function readMarkdownOutput(outputPath, stdout) {
    let output = '';
    try {
...
export async function convertWithMarkItDown(input) {
    ...
    try {
        ...
        if (error && typeof error === 'object' && 'error' in error) {
            return classifyExecFailure(error, tempDir);
        }
