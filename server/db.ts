...
export type ContactUpdateInput = Partial<Omit<ContactInput, 'active'>> & {
  active?: boolean;
...
function mapPreferenceSet(row: any): PreferenceSet {
  return {
    id: numericValue(row.id),
    roleKey: String(row.role_key),
    name: String(row.name),
...
function mapRole(row: any, preferenceSets: PreferenceSet[] = []): RoleRow {
  return {
    key: String(row.key),
...
async function roleRows() {
  ...
  const byRole = new Map<string, PreferenceSet[]>();
...
export const repo = {
  ...
  async createPreferenceSet(roleKey: RoleKey, input: PreferenceSetInput) {
  ...
  async updatePreferenceSet(id: number, input: Partial<PreferenceSetInput>) {
    const existing = await repo.preferenceSet(id);
    if (!existing) throw notFound('偏好方案不存在');
    await db.execute({
  ...
  async deletePreferenceSet(id: number) {
    const preferenceSet = await repo.preferenceSet(id);
    if (!preferenceSet) throw notFound('偏好方案不存在');
    if (preferenceSet.usageCount > 0) throw conflict(`该偏好方案仍被 ${preferenceSet.usageCount} 位联系人使用，请先更换联系人偏好`);
  ...
  async validateContactConfiguration(contact: Pick<Contact, 'roleMode' | 'roleKey' | 'rolePreferenceId' | 'customRoleLabel' | 'customRolePreference'>) {
    if (contact.roleMode === 'custom') {
      if (!contact.customRoleLabel.trim()) return '联系人专属角色名称不能为空';
      if (!contact.customRolePreference.trim()) return '联系人专属角色偏好不能为空';
      return null;
    }
    const role = await repo.role(contact.roleKey);
    if (!role) return '所选角色不存在';
    if (contact.rolePreferenceId != null) {
      const preferenceSet = await repo.preferenceSet(contact.rolePreferenceId);
      if (!preferenceSet || preferenceSet.roleKey !== role.key) return '所选偏好方案不属于当前角色';
    }
    return null;
  },
  async resolveRoleForContact(contact: Contact) {
    if (contact.roleMode === 'custom') {
      return {
    ...
    if (preferenceSet && preferenceSet.roleKey !== role.key) return null;
  ...
  async createSendRecord(input: { generationRecordId: number | null; contactId: number; deliveryType?: DeliveryType; webhookUrl: string; payload: unknown; responseStatus?: number; responseBody?: string; error?: string }) {
    const result = await db.execute({ sql: `
