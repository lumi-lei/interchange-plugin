...
export async function readSkillFrontmatter(filePath: string, fallbackName: string): Promise<{ name: string; description: string }> {
  let text = '';
  try {
...
async function listSkillEntries(root: string): Promise<{ name: string; description: string; sourceDir: string }[]> {
  let entries: string[] = [];
  try {
  ...
  const result: { name: string; description: string; sourceDir: string }[] = [];
...
export async function discoverCatalog(): Promise<DsgCatalog> {
  ...
  const skills = new Map<string, DsgSkillItem>();
...
export async function resolveSkillSourceDir(name: string): Promise<string | null> {
  const sources = [
    { root: repoSkillsDir(), source: 'interchange' as const },
    { root: dshSkillsDir(), source: 'global' as const },
    { root: projectSkillsDir(), source: 'project' as const },
  ];
  for (const { root } of sources) {
    ...
    try {
      if ((await stat(flat)).isFile()) return flat;
    } catch {}
  }
  return null;
}
