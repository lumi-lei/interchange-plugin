...
const PERSONA = (roleLabel: string, defaultPreference: string) => {
...
function DELEGATION({ subagents, workflow, ralph }: { subagents: boolean; workflow: boolean; ralph: boolean }) {
  const rows: string[] = [];
  if (subagents) {
...
function INTERCHANGE_TOOLS(workspaceDir: string) {
...
export type DsgPresetSelection = {
  roleLabel: string;
  defaultPreference: string;
  workspaceDir: string;
};

export function renderAgentCordis(selection: DsgPresetSelection): string {
  const blocks: string[] = [];
...
export function renderPresetMeta(roleLabel: string, skillCount: number): string {
